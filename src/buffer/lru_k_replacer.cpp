...
namespace bustub {
auto LRUKReplacer::Comparator::operator()(const LRUKNode *l, const LRUKNode *r) const -> bool {
  if (l->history_.size() == k_ && r->history_.size() < k_) {
    return false;
  }
  if (l->history_.size() < k_ && r->history_.size() == k_) {
    return true;
...
LRUKReplacer::LRUKReplacer(size_t num_frames, size_t k) : evict_queue_(Comparator{k}), max_size_(num_frames), k_(k) {}

auto LRUKReplacer::Evict(frame_id_t *frame_id) -> bool {
  std::lock_guard<std::mutex> guard(latch_);
  auto it = evict_queue_.begin();
  if (it == evict_queue_.end()) {
    return false;
  }
  *frame_id = (*it)->fid_;
...
void LRUKReplacer::RecordAccess(frame_id_t frame_id, AccessType access_type) {
  ...
  if (node_store_.size() < max_size_) {
    auto &node = node_store_.emplace(frame_id, LRUKNode()).first->second;
    node.fid_ = frame_id;
    node.history_.push_front(current_timestamp_);
    node.is_evictable_ = true;
    node.iter_ = evict_queue_.emplace(&node).first;
    ++evictable_size_;
    return;
  }
  throw("error");
